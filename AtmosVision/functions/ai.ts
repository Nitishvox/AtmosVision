...
3.  **Two Response Modes**:
    a. **Detailed Analysis**: For general questions about named locations (e.g., "What are the risks in Cairo?"), you MUST provide a structured, detailed analysis. Follow this format: start with an intro sentence, then "### Top 3 Critical Risks" (numbered list), then "### Key Preparedness Actions" (bulleted list), and finally "### Data Sources" (bulleted list). Append the mandatory 
```json block for location data if you can identify a specific place.
...
export const streamMessage = async (req: Request, res: Response) => {
  if (!ai) {
    return res.status(500).json({ error: "AI client is not configured. API key is missing." });
  }

  const { message, activeLayers, userLocation, image } = req.body;
  ...
  try {
    ...
    res.status(500).json({ error: "I'm sorry, I'm having trouble connecting to my knowledge base right now. Please try again later." });
  }
};
